...
function manejoDOM() {
     ...
     var obtenidos = JSON.parse(localStorage.getItem('array'));


     for (const obtenido of obtenidos) {
        var contenedor = document.createElement("div");
        contenedor.innerHTML = `<div class="card" style="width: 18rem; margin-bottom: 8px; margin-left: 30px;">
        <div class="card-body">
          <h5 class="card-title">${obtenido.tipo} ${obtenido.marca} </h5>
          <h6 class="card-subtitle mb-2 text-muted">Variedad: ${obtenido.variedad} </h6>
          <p class="card-text">El precio de esta bebida es ${obtenido.precio}  </p>
        </div>
      </div>`;
